...
class TA(object):
    ...
    def make_macd_data(self, dataset, s=12, l=26, m=9):  # MACD数据一定要长时间的数据 30个数据点不够, 至少需要90, 这样数据才准
        try:
            ...
            self.log.logger.error("add macd data error: %s" % err.message)

    def make_macd_cross(self, dataset):
        try:
            dataset['macd_cross'] = ""
            ...
            self.log.logger.error("add macd cross data error: %s" % err.message)

    def make_kdj_data(self, dataset, n1=9, n2=3):
        try:
            ...
            self.log.logger.error("add kdj data error: %s" % err.message)

    def make_kdj_cross(self, dataset):
        try:
            dataset['kdj_cross'] = ""
            ...
            self.log.logger.error("add kdj cross data error: %s" % err.message)

    def make_change_data(self, dataset):
        try:
            dataset['change'] = dataset['close'].diff()  # 计算价格偏差
            dataset['pct_change'] = dataset['close'].pct_change() * 100  # 计算百分比
        except Exception as err:
            self.log.logger.error("add change data error: %s" % err.message)

    def make_ma_data(self, dataset):
        try:
            ...
            self.log.logger.error("add ma data error: %s" % err.message)

    def make_rsi_data(self, dataset, n1=6, n2=12, n3=24):
        try:
            ...
            self.log.logger.error("add rsi data error: %s" % err.message)

    def make_rsi_cross(self, dataset):
        try:
            dataset['rsi_cross'] = ''
            ...
            self.log.logger.error("add rsi cross data error: %s" % err.message)

    def make_cci_data(self, dataset):
        try:
            dataset['cci'] = talib.CCI(dataset['high'].values, dataset['low'].values, dataset['close'].values)
        except Exception as err:
            self.log.logger.error("add cci data error: %s" % err.message)
